...
int main(int argc, char* argv[]) {
  ...
  sprintf(outputDir, "%s/optResults_%c/phiOffset%02.1f/thOffset%06.4f", inputDir, whichPol, hpPhiOffset, hpThOffset);
  ...
  lprintf("rotated cut slope value = %f \n", cutSlope);
  ...
  else // if we are loading the spillover data:
  {
    ...
    char spillDir[1024];
    sprintf(spillDir, "%s/spillover/phiOff%03.4f/thOff%03.4f", inputDir, hpPhiOffset, hpThOffset);
  ...
  for (map<int, dataRow_t>::iterator thisPair=cutValHist.begin(); thisPair!=cutValHist.end(); ++thisPair) {      
    ...
    if (thisHist->GetSumOfWeights() > 0) {
      ...
      // make a fit to the histogram from maximBin+1 to last descending bin
      thisHist->Sumw2();
      //if (descBinCount >= 4) {                 // try keeping bins with less data?
      if (descBinCount >= 5) { 
        ...
        float thisCovM00 = mmin.UserCovariance()(0,0);
        float thisCovM01 = mmin.UserCovariance()(0,1);
        float thisCovM11 = mmin.UserCovariance()(1,1);
        ...
        */
        ...
        //But!! we still want the fit on the plot, so lets tack it on! (Jacob!)
  ...
  for (map<int, dataRow_t>::iterator thisPair=cutValHist.begin(); thisPair!=cutValHist.end(); ++thisPair) {
    ...
    if (thisRow.status == 0) {
      ...
      for (int pe = 0; pe<pseudoExpCount; pe++) {
        ...
        // Make out data vectors
        vector<double> pos_pse, meas_pse, var_pse;
        ...
        /*
        if (thisPair->first == ourBin) {
          //lprintf("   bin %i pseudo-exp %i, log-likelihood = %f \n", thisPair->first, pe, l0/thisPseHist->GetSumOfWeights());
          if (false) {
            TCanvas* pseCanv = new TCanvas("pseCanv", "pseCanv", 600, 600);
            pseCanv->cd(0);
            gPad->SetLogy();
            thisPseHist->Draw();
            gStyle->SetOptStat("");
  ...
  sprintf(inFilename, "%s/analysisOutput_2_sim.root", inputDir);
  ...
  */
  ...
  TFile* simCountFile = new TFile("results/plots/simCountFile_801_808.root");
  ...
  for (int gBin = 0; gBin < binStatusHist->GetNcells()-1; ++gBin) {
    ...
    for (int i=0; i<vecMap.size(); i++)
    {
      ...
      if ( vecMap[i].second.binStatus == 0 )
  ...
  binStatusL->AddEntry( dummyBox2,"< 5 Data points to Fit", "F");              // binStatus = 1
  ...
  //for (float scaleFac = startScaleFac; scaleFac > endScaleFac; scaleFac /= scaleFacInc) 
  {
    ...
    if ( i == 0 )
    {
      sprintf(filename, "%s/optimization_sl_%02.0f_sf_%01.6f.txt", outputDir, -cutSlope, scaleFac);
    } else {      
      sprintf(filename, "%s/optimization_final_sl_%02.0f_sf_%01.6f.txt", outputDir, -cutSlope, scaleFac);
    }
  ...
  //char filename[1024];
  sprintf(filename, "%s/optSummary_%2.1f_%4.3f_sl_%02.0f.txt", outputDir, circPeakSepThreshold, circPeakStrengthThreshold, -cutSlope);
  FILE* summaryFile;  
