...
// -Output to file.
//
// - 1/24/18 -- updated to use new method for shifting healpix bins which does not
//              skew the shapes of the bins.
//
////////////////////////////////////////////////////////////////////////
...
int main(int argc, char* argv[]) {
  ...
  if ( doRebinning )
  {
    ...
    for (int e = index; e<tree0Index->GetN(); e+=10)
    {
      ...
      for (int k=0; k<errHexPixels[0].size(); ++k) {
        ...
        double thisTheta = thisPoint.first;
        double thisPhi = thisPoint.second;
  ...
  //for (int e=0; e<10; ++e) {
    ...
    vector<vector<int> > hpBinA(100,vector<int>(100,0));
    vector<vector<double> > eaA(100,vector<double>(100,0));
    vector<vector<double> > noA(100,vector<double>(100,0));
    vector<vector<float> > newProbA(100,vector<float>(100,0));
    ...
    for (int xi = 0; xi < nSteps; xi++)
    {
      ...
      for (int yi = 0; yi < nSteps; yi++)
      {
        ...
        if (thisLat == -9999 || thisLon == -9999 || thisAlt == -9999) { thisHpBin = 0; }
          ...
          pair<double,double> thisEPoint = rotateOnSphere((90.0-thisLat)*M_PI/180.0, thisLon*M_PI/180.0, hpPhiOffset*M_PI/180.0, hpThOffset*M_PI/180.0);
          double thisETheta = thisEPoint.first;
          double thisEPhi = thisEPoint.second;
          ...
          // while here lets grab the ea and no
          bedmap->LonLattoEaNo(thisLon, thisLat,  thisEa, thisNo);
        }
        
        //for each gridpoint (that has enough prob) save its hpBin, its ea, and its no.
        
        hpBinA[xi][yi] = thisHpBin;
        eaA[xi][yi] = thisEa;
        noA[xi][yi] = thisNo;
      }
    }
    // leave grid loop and start new grid loop
    //   new grid loop has a special normalization for the 
    //   area a grid point represents on the map in ea and no.
    for (int xi = 0; xi < nSteps; xi++)
    {
      for (int yi = 0; yi < nSteps; yi++)
      {
        ...
        // If the grid point doesnt reconstruct to the continent we cant get an area
        //  I guess thats like saying the area is 0, so pass on its P as 0?
        if ( hpBinA[xi][yi] == 0 ) 
        {
          newProbA[xi][yi] = 0;   //thisP;
          continue;
        }

        // find area this grid point represents.
        double xL = 0;
        double yL = 0;
        int plusXFlag = 1;   // flags to see if there are grid points around our current grid point.
        int minusXFlag = 1;
        int plusYFlag = 1;
        int minusYFlag = 1;

        //check for data around x
        if( xi==0 ) {   minusXFlag = 0;   }
        else if( xi==99 ) {   plusXFlag = 0;   }
        else {
          if( hpBinA[xi-1][yi] == 0 ) {   minusXFlag = 0;   }
          if( hpBinA[xi+1][yi] == 0 ) {   plusXFlag = 0;   }
        }

        //check for data around y
        if( yi==0 ) {   minusYFlag = 0;   }
        else if( yi==99 ) {   plusYFlag = 0;   }
        else {
          if( hpBinA[xi][yi-1] == 0 ) {   minusYFlag = 0;   }
          if( hpBinA[xi][yi+1] == 0 ) {   plusYFlag = 0;   }
        }

        // find xL
        if ( plusXFlag == 1 && minusXFlag == 1 )
        {
          xL = 0.5*(  sqrt( (eaA[xi][yi]-eaA[xi-1][yi])*(eaA[xi][yi]-eaA[xi-1][yi]) 
                           +(noA[xi][yi]-noA[xi-1][yi])*(noA[xi][yi]-noA[xi-1][yi]) )
                     +sqrt( (eaA[xi][yi]-eaA[xi+1][yi])*(eaA[xi][yi]-eaA[xi+1][yi])
                           +(noA[xi][yi]-noA[xi+1][yi])*(noA[xi][yi]-noA[xi+1][yi]) ) 
                    );
        }
        else if ( plusXFlag == 1 )
        {
          xL = sqrt( (eaA[xi][yi]-eaA[xi+1][yi])*(eaA[xi][yi]-eaA[xi+1][yi])
                    +(noA[xi][yi]-noA[xi+1][yi])*(noA[xi][yi]-noA[xi+1][yi]) );
        }
        else if ( minusXFlag == 1 )
        {
          xL = sqrt( (eaA[xi][yi]-eaA[xi-1][yi])*(eaA[xi][yi]-eaA[xi-1][yi])
                    +(noA[xi][yi]-noA[xi-1][yi])*(noA[xi][yi]-noA[xi-1][yi]) );
        }
        ...
        {
          //well... damn.  move twoards the middle and try there?
          int xDir = -1; int yDir = -1;
          if (xi < 50) { yDir = 1; }
          if (yi < 50) { xDir = 1; }
          
          xL = sqrt( (eaA[xi][yi+yDir]-eaA[xi+xDir][yi+yDir])*(eaA[xi][yi+yDir]-eaA[xi+xDir][yi+yDir])
                    +(noA[xi][yi+yDir]-noA[xi+xDir][yi+yDir])*(noA[xi][yi+yDir]-noA[xi+xDir][yi+yDir]) );
        }

        // find yL
        if ( plusYFlag == 1 && minusYFlag == 1 )
        {
          yL = 0.5*(  sqrt( (eaA[xi][yi]-eaA[xi][yi-1])*(eaA[xi][yi]-eaA[xi][yi-1])
                           +(noA[xi][yi]-noA[xi][yi-1])*(noA[xi][yi]-noA[xi][yi-1]) )
                     +sqrt( (eaA[xi][yi]-eaA[xi][yi+1])*(eaA[xi][yi]-eaA[xi][yi+1])
                           +(noA[xi][yi]-noA[xi][yi+1])*(noA[xi][yi]-noA[xi][yi+1]) )
                    );
        }
        else if ( plusYFlag == 1 )
        {
          yL = sqrt( (eaA[xi][yi]-eaA[xi][yi+1])*(eaA[xi][yi]-eaA[xi][yi+1])
                    +(noA[xi][yi]-noA[xi][yi+1])*(noA[xi][yi]-noA[xi][yi+1]) );
        }
        else if ( minusYFlag == 1 )
        {
          yL = sqrt( (eaA[xi][yi]-eaA[xi][yi-1])*(eaA[xi][yi]-eaA[xi][yi-1])
                    +(noA[xi][yi]-noA[xi][yi-1])*(noA[xi][yi]-noA[xi][yi-1]) );
        }
        else
        {
          ...
          yL = sqrt( (eaA[xi+xDir][yi]-eaA[xi+xDir][yi+yDir])*(eaA[xi+xDir][yi]-eaA[xi+xDir][yi+yDir])
                    +(noA[xi+xDir][yi]-noA[xi+xDir][yi+yDir])*(noA[xi+xDir][yi]-noA[xi+xDir][yi+yDir]) );
        }

        thisP *= xL*yL;
        newProbA[xi][yi] = thisP;
      }
    }
    float newPSum = 0;
    for (int xi = 0; xi < nSteps; xi++)
    {
      for (int yi = 0; yi < nSteps; yi++)
      {
        newPSum += newProbA[xi][yi];
      }
    }
    //cout << "sum = " << newPSum << endl;
    for (int xi = 0; xi < nSteps; xi++)
    {
      for (int yi = 0; yi < nSteps; yi++)
      {
        // use old P to skip edges
        if (probArray[xi][yi] < minProb) {continue;}

        // Find the FINAL P for this grid point
        double thisP = newProbA[xi][yi];
        thisP /= newPSum;                             // normalize to 1
        thisP *= hpWeight;                            // normalize by hpWeight

        int thisHpBin = hpBinA[xi][yi];
  ...
  if( makePlots )
  {
    ...
    for (int e = 0; e<tree0Index->GetN(); e+=1)
    {
      ...
      pair<double,double> thisEPoint = rotateOnSphere((90.0-lat)*M_PI/180.0, lon*M_PI/180.0, hpPhiOffset*M_PI/180.0, hpThOffset*M_PI/180.0);
      double thisETheta = thisEPoint.first;
      double thisEPhi = thisEPoint.second;
    ...
    for (pair<int, int> thisEntry : binCount) {
      ...
      double thisLat = 90.0 - (thisPointNew.first * 180.0/M_PI);
      double thisLon = thisPointNew.second * 180.0/M_PI;
